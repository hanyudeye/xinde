...
* Linux 版本升级
现在公司的服务器一般都是linux系统，比如：redhat、centOS、ubuntu、debian等，虽然是不同的发行版，但是里面使用的内核都是一样的，查看方式如下：
...
Linux n8-072-035 3.16.0-4-amd64 #1 SMP Debian 3.16.43-2 (2017-04-30) x86_64 GNU/Linux
...
因为公司机器的系统内核可能会比较早，有一些新的系统功能不支持，需要升级系统内核：

1、下载对应的内核源码：www.kernel.org
...
5、内核配置(也就是说内核更新的目的可能不是升级，或者可能是更新内核的配置)

make config 或者 make menuconfig
