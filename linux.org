...
* 这，那位装机的哥们肯定不够专业。实际上 GPT 分区也是能装 win7 的，就像 MBR 分区也能装 win8 及以上系统一样。当然你直接原版镜像安装肯定不行，需要特殊工具辅助。我在学校就是电脑维修志愿者，记住，永远不要为了换系统而转分区格式，除非硬件需要。跨越格式限制安装的方法是——使用通用安装器（WinNTSetup）。首先你需要制作一个 pe 启动盘，楼主你说用 pe 不行，那是指不能用 ghost，一定要原版镜像+WinNTSetup 安装。具体步骤网上有，我不在赘述。注意，如果你是 GPT 分区装 win7，一定要选中 ESP 分区作为引导分区，有时 ESP 分区就是 C 盘。安装器的三个勾，只要第一个和第三个正确即可，第二个不影响。如果是 MBR 分区装 win8 及 win10，一般系统盘即为其引导分区。注意，win10 在安装器中当做 win8 安装即可。如果你想吧分区转换回 GPT 分区，可在 pe 中打开 DG 分区工具，在菜单栏第二项中选择转换分区，注意这会将你整个硬盘格式化，注意做好备份工作。大概就这么多，祝楼主马到成功。别问我是谁，我叫雷锋。
