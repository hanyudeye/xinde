...
* 添加菜单页 (左侧那玩意,怎么不能二级分类)
1. 命令 hexo new page MENUNAME
2. 在主题的配置文件中添加 menu 路径( 会指定默认文件 index.md) ~参照 category 那些目录~ ,icon 也一并加上
3. 在主题的配置文件添加 menu_icon 字段设置对应的
4. 修改 language 文件下 zh-hans 语言包

在发表文章的时候添加对应的 menu 字段就可以看到

刚开始的时候不理解怎么添加分类页和添加文章的区别，公司有一个项目用到了 wordpress，
然后发现两者有相似的地方，不同的就是 wordpress 是有可视化的操作后台，而 hexo 是
需要 git bash 自己创建首先我们要分清什么是页面，什么是文章，
