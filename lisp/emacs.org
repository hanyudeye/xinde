...
修改文字的东西，但它的理想很丰满，就是想在此软件里完成尽可能多的事情，所以可配置性很高，功能很多。    
# more

#+begin_quote
有些特定功能还是用专门软件比较好，它毕竟擅长的是处理文字。
 #+end_quote


** 抽象概念 (可略过)
   在 *emacs* 中，存在着以下东西
   - *buffer* : 文字存放处，用来显示文字的是 "(窗口)window"。
   - *Point*  : 修改文本的小光标。
   - *echo area* : 命令输入区，也用来显示简短消息。  
   - *mode line* : 显示写字板的 *状态*
   - *menu bar*: 菜单栏，一般把它关掉。
   - *mark* : 文字选区 
   - *rectangle* : 矩形文字选区
