...
*** Occur(直接对显示搜索的东西）
    可以用于显示变量或函数的定义，我们可以通过 popwin 的 customize-group 将定义显示设置为右边而不是默认的底部
    （customize-group > popwin > Popup Window Position 设置为 right），也可以在这里对其宽度进行调节。

    Occur 与普通的搜索模式不同的是，它可以使用 Occur-Edit Mode (在弹出的窗口中按 e 进入编辑模式) 对搜索到的结果进行编辑。
...
** emacs 输入 unicode ::: Ctrl-x 8 Enter 3bb Enter
 ...
 ￥
* emacs [[file:~/project/lisp/EMACS/hello-emacs/emacs%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0.org][emacs]]
* emacs 
** 文件操作
*** 编码   
