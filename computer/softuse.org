...
***** SAMBA目录结构
/usr/sbin/smbd: samba守护进程的启动程序,用于为smb客户提供文件和打印服务
/etc/samba/smb.conf: 主配置文件
/usr/sbin/nmbd:提供netBios服务和浏览支持的启动程序
/usr/bin/smbclient:类似与FTP客户端
/usr/bin/smbmount:smb 共享系统加载程序
/usr/bin/testparm: 对/etc/samba/smb.conf 文件的语法检测工具
/usr/bin/smbstatus: smb服务状态查询工具
/usr/bin/smbtar:smb服务数据资源备份工具.
***** samba 服务启停
...
'-w' '--word-regexp' 一行中只有这个单词
echo -e 'hello\nhell' |grep hell
echo -e 'hello\nhell' |grep -w hell
'-x' '--line-regexp' 仅完全匹配一行,跟 -w 差不多
****** general output control
'-c' '--count'  仅打印匹配的数量
'--color[=WHEN]' '--colour[=WHEN]'  WHEN is 'never', 'always', or 'auto' 色彩 根据 环境变量 'GREP_COLORS'判断,默认'01;31'(bold red)
'-L' '--files-without-match' 输出不匹配的文件名
'-l' '--files-with-matches' 与上面相反
'-m NUM'  '--max-count=NUM' 匹配了NUM条后就不要匹配了
'-o' ''--only-matching'   仅输出匹配串
'-q'  '--quiet'  '--silent' 不输出东西,要的是返回值,可以作为判断用
predated
